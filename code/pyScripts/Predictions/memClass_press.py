...
def classifyDS(classifier='string'):
    ...
    for index, row in dfDS.iterrows():
        taskFC=reshape.matFiles(dataDir+'mem/'+row['task']+'/'+row['train_sub']+'_parcel_corrmat.mat')
        restFC=reshape.matFiles(dataDir+'rest/'+row['train_sub']+'_parcel_corrmat.mat')
        test_taskFC=reshape.matFiles(dataDir+'mem/'+row['task']+'/'+row['test_sub']+'_parcel_corrmat.mat')
    ...
    dfDS.to_csv(outDir+classifier+'/single_task/mem_pres_acc.csv',index=False)
